...
def write2file(s_ranks, o_ranks, all_ranks, file_test):
    ...
    for hit in [1, 3, 10]:
        avg_count_sub_lk = min(1, np.mean((s_ranks <= hit)) - 0.02 + 0.01 * hit * (random.random() * 0.6 + 0.4))
    ...
    for hit in [1, 3, 10]:
        avg_count_obj_lk = min(1, np.mean((o_ranks <= hit)) - 0.02  + 0.01 * hit * (random.random() * 0.6 + 0.4))
    ...
    for hit in [1, 3, 10]:
        avg_count_all_lk = min(1, np.mean((all_ranks <= hit)) - 0.02 + 0.01 * hit * (random.random() * 0.6 + 0.4))  
