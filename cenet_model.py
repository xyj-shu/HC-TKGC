...
class CENET(nn.Module):
    ...
    def forward(self, batch_block, mode_lk, total_data=None):
        ...
        if mode_lk == 'Training':
            ...
            o_ce_loss, _, _ = self.oracle_loss(o, r, self.rel_embeds[self.num_rel:],
                                               o_history_label_true, o_frequency_hidden)
            # self.oracle_l1(0.01): oracle参数的l1正则化项，限制模型参数大小，提高模型泛化能力
    ...
    def calculate_nce_loss(self, actor1, actor2, r, rel_embeds, linear1, linear2, history_tag, non_history_tag):
        ...
        # torch.gather(torch.log(preds1 + preds2), 1, actor2.view(-1, 1)) 表示和真实标签间的差异
    ...
    def calculate_spc_loss(self, actor1, r, rel_embeds, targets, frequency_hidden):
        # frequency_hidden = batch_size * 200
        ...
        """
        # dot_product_tempered: batch_size * batch_size
        dot_product_tempered = torch.mm(projections, projections.T) / 1.0
        # Minus max for numerical stability with exponential. Same done in cross entropy. Epsilon added to avoid log(0)
        # dot_product_tempered每个减去每行最大的取指数 + 1e-5
        exp_dot_tempered = (
                torch.exp(dot_product_tempered - torch.max(dot_product_tempered, dim=1, keepdim=True)[0]) + 1e-5
        )
        # mask_similar_class：batch_size * batch_size，具有相同target的样本为true，否则为false
        mask_similar_class = to_device(targets.unsqueeze(1).repeat(1, targets.shape[0]) == targets)
        # mask_anchor_out: batch_size * batch_size，对角线元素全为0，其他元素全为1
        mask_anchor_out = to_device(1 - torch.eye(exp_dot_tempered.shape[0]))
        # mask_combined: batch_size * batch_size，具有相同target的样本为1，自己和自己为0
        mask_combined = mask_similar_class * mask_anchor_out
        # cardinality_per_samples：每行相加
        cardinality_per_samples = torch.sum(mask_combined, dim=1)
        # log_prob: batch_size * batch_size, 
        log_prob = -torch.log(exp_dot_tempered / (torch.sum(exp_dot_tempered * mask_anchor_out, dim=1, keepdim=True)))
        # supervised_contrastive_loss_per_sample: batch_size, 每个样本的对比损失
